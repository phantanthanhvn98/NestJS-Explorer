...
import type { SortType } from 'plus0-sdk';
...
class BaseModelSqlv2 {
  ...
  async delByPk(id, _trx?, cookie?) {
    ...
    try {
      const source = await Source.get(this.model.dataset_id);
  ...
  async bulkDelete(
    ...
    try {
      ...
      const base = await Source.get(this.model.dataset_id);
      ...
      if (col.uidt === UITypes.Formula) {
        ...
        d[col.id] = d[col.id].replace(
          ...
          (d: string) => {
            ...
            if (this.isSqlite) {
              // if there is no timezone info,
              // we assume the input is on plus0-service server timezone
