...
import { Plus0Config } from 'src/utils/plus0-config';
...
export class MetaService {
  ...
  constructor(config: Plus0Config, @Optional() trx = null) {
    this._config = config;
    this._knex = XKnex({
  ...
  public async metaInsert2(
    ...
    if (dataset_id !== null) insertObj.dataset_id = dataset_id;
  ...
  public async bulkMetaInsert(
    ...
    if (dataset_id !== null) commonProps.dataset_id = dataset_id;
    if (project_id !== null) commonProps.project_id = project_id;
  ...
  public async genNanoid(target: string) {
    ...
    // Fustion Auth only accept uuid4
    return  MetaTable.USERS == target ? `${uuid4().toString()}`:`${prefix}-${uuid4().toString()}`;
  }
  /***
  ...
  public async metaPaginatedList(
    ...
    if (baseId !== null && baseId !== undefined) {
      query.where('project_id', baseId);
      countQuery.where('project_id', baseId);
    }
    if (dbAlias !== null && dbAlias !== undefined) {
      query.where('db_alias', dbAlias);
  ...
  public async metaGet2(
    ...
    if (sourceId !== null && sourceId !== undefined) {
      query.where('dataset_id', sourceId);
    }
  ...
  public async metaList2(
    ...
    if (dbAlias !== null && dbAlias !== undefined) {
      query.where('dataset_id', dbAlias);
    }
  ...
  public async baseUpdate(baseId: string, config: any): Promise<any> {
    ...
    try {
      ...
      await this.knexConnection(MetaTable.PROJECT).update(base).where({
        id: baseId,
      });
    } catch (e) {
      console.log(e);
  ...
  public async baseList(): Promise<any[]> {
    return (await this.knexConnection(MetaTable.PROJECT).select()).map((p) => {
