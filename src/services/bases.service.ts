...
import { getToolDir } from 'src/utils/plus0-config';
...
export class BasesService {
  ...
  async baseList(param: {
    ...
    const bases = param.user?.roles == OrgUserRoles.SUPER_ADMIN
  ...
  async baseCreate(param: { base: ProjectReqType; user: any; req: NcRequest }) {
    ...
    if (!baseBody.external) {
      const ranId = nanoid();
      baseBody.prefix = `plus0_${ranId}__`;
      ...
      if (process.env.NC_MINIMAL_DBS === 'true') {
        ...
        // each file will be named as plus0_<random_id>.db
        const fs = require('fs');
