...
export class ColumnsService {
  ...
  async columnUpdate(param: {
    ...
    if (
      ...
      const sqlMgr = await reuseOrSave('sqlMgr', reuse, async () =>
        ProjectMgrv2.getSqlMgr({ id: source.dataset_id }),
      );
      await sqlMgr.sqlOpPlus(source, 'tableUpdate', tableUpdateBody);
      ...
      if (column.uidt === UITypes.User) {
        ...
        const sqlMgr = await reuseOrSave('sqlMgr', reuse, async () =>
          ProjectMgrv2.getSqlMgr({ id: source.project_id }),
        );
        await sqlMgr.sqlOpPlus(source, 'tableUpdate', tableUpdateBody);
  ...
  async columnDelete(
    ...
    const source = await reuseOrSave('source', reuse, async () =>
      Source.get(table.dataset_id, false, ncMeta),
    );

    const sqlMgr = await reuseOrSave('sqlMgr', reuse, async () =>
      ProjectMgrv2.getSqlMgr({ id: source.project_id }, ncMeta),
    );
  ...
  async columnBulk(
    ...
    const source = await Source.get(table.dataset_id);

    if (!source) {
      NcError.sourceNotFound(table.dataset_id);
    }

    const base = await source.getProject();

    if (!base) {
      NcError.baseNotFound(source.project_id);
    }

    const dbDriver = await NcConnectionMgrv2.get(source);
    const sqlClient = await NcConnectionMgrv2.getSqlClient(source);
    const sqlMgr = await ProjectMgrv2.getSqlMgr({ id: source.project_id });
