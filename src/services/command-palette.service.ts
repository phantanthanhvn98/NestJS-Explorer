...
import { type UserType, ViewTypes } from 'plus0-sdk';
      ...
      for (const v of viewList) {
        if (!tableList.find((el) => el.id === `tbl-${v.fk_model_id}`)) {
          tableList.push({
            ...
            title: v._ptn,
            parent: `p-${v.project_id}`,
            icon: v?.table_meta?.icon || v.ptype,
            projectName: bases.find((el) => el.id === v.project_id)?.title,
            section: 'Tables',
          });
        }
        vwList.push({
          ...
          handler: {
            type: 'navigate',
            payload: `/nc/${v.project_id}/${v.fk_model_id}/${encodeURIComponent(
              v.id,
            )}`,
          },
        });
