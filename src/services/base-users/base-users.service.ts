...
import type { ProjectUserReqType, UserType } from 'plus0-sdk';
...
export class BaseUsersService {
  ...
  async baseUserUpdate(param: {
    userId: string;
    // todo: update swagger
    baseUser: ProjectUserReqType & { project_id: string };
    // todo: refactor
    req: any;
    baseId: string;
  }): Promise<any> {
  ...
  async baseUserDelete(param: {
    ...
    if (!param.req.user?.base_roles?.owner) {
      ...
      const baseUser = await BaseUser.get(project_id, param.userId);
      if (baseUser?.roles?.split(',').includes('owner'))
        NcError.forbidden('Insufficient privilege to delete a owner user.');
    }

    await BaseUser.delete(project_id, param.userId);
    return true;
  }
