...
import { ViewTypes } from 'plus0-sdk';
...
import { jdbcToXcConfig } from 'src/utils/plus0-config/helpers';
  ...
  async versionInfo() {
    if (
      ...
      const nonBetaTags = await axios
        .get('https://api.github.com/repos/plus0-service/plus0-service/tags', {
          timeout: 5000,
        })
        .then((response) => {
