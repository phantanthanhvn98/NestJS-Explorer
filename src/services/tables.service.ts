...
export class TablesService {
  ...
  async tableUpdate(param: {
    ...
    const base = await Base.getWithInfo(param.table.project_id || param.baseId);
    const source = base.sources.find((b) => b.id === model.dataset_id);

    if (model.project_id !== base.id) {
  ...
  async tableDelete(param: {
    ...
    const base = await Base.getWithInfo(table.project_id);
    const source = base.sources.find((b) => b.id === table.dataset_id);
