...
import { isLinksOrLTAR, RelationTypes, UITypes, ViewTypes } from 'plus0-sdk';
...
export class ExportService {
  ...
  async serializeModels(param: {
    ...
    for (const modelId of modelIds) {
      ...
      const fndProject = bases.find((p) => p.id === model.project_id);
      const base = fndProject || (await Base.get(model.project_id));

      const fndBase = sources.find((b) => b.id === model.dataset_id);
      const source = fndBase || (await Source.get(model.dataset_id));
            ...
            switch (k) {
              ...
              case 'fk_target_view_id':
                if (v) {
                  ...
                  idMap.set(
                    view.id,
                    `${source.project_id}::${source.id}::${getEntityIdentifier(
                      view.fk_model_id,
                    )}::${view.id}`,
                  );
                  column.colOptions[k] = idMap.get(v as string);
  ...
  async streamModelDataAsCsv(param: {
    ...
    for (const column of model.columns.filter(
      ...
      if (fkCol) {
        ...
        btMap.set(
          fkCol.id,
          `${column.project_id}::${column.dataset_id}::${column.fk_model_id}::${column.id}`,
        );
      }
    }
    ...
    const formatData = (data: any) => {
      for (const row of data) {
        for (const [k, v] of Object.entries(row)) {
          ...
          if (col) {
            const colId = `${col.project_id}::${col.dataset_id}::${col.fk_model_id}::${col.id}`;
            switch (col.uidt) {
    ...
    if (hasLink) {
      ...
      for (const mm of mmColumns) {
        ...
        const mmBase =
          mmModel.dataset_id === source.id
            ? source
            : await Source.get(mmModel.dataset_id);
  ...
  async exportBase(param: { path: string; sourceId: string }) {
    ...
    elapsedTime(
      hrTime,
      `serialize models for ${source.project_id}::${source.id}`,
      'exportBase',
    );
    ...
    try {
      ...
      elapsedTime(
        hrTime,
        `export source ${source.project_id}::${source.id}`,
        'exportBase',
      );
    } catch (e) {
