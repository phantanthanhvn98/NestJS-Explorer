...
import { isLinksOrLTAR, isVirtualCol, RelationTypes } from 'plus0-sdk';
...
export class DuplicateProcessor {
  ...
  async duplicateBase(job: Job) {
    ...
    try {
      ...
      elapsedTime(
        hrTime,
        `serialize models schema for ${source.project_id}::${source.id}`,
        'duplicateBase',
      );
  ...
  async duplicateColumn(job: Job) {
    ...
    const source = await Source.get(sourceColumn.dataset_id);
