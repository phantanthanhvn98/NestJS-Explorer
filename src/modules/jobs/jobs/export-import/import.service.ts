...
import type { UserType, ViewCreateReqType } from 'plus0-sdk';
  ...
  async importModels(param: {
    ...
    if (param.externalModels) {
      for (const model of param.externalModels) {
        externalIdMap.set(
          `${model.project_id}::${model.dataset_id}::${model.id}`,
          model.id,
        );
        ...
        if (primaryKey) {
          idMap.set(
            `${model.dataset_id}::${model.dataset_id}::${model.id}::${primaryKey.id}`,
            primaryKey.id,
          );
        }
        ...
        for (const col of model.columns) {
          externalIdMap.set(
            `${model.dataset_id}::${model.dataset_id}::${model.id}::${col.id}`,
            col.id,
          );
        }
        ...
        for (const view of model.views) {
          externalIdMap.set(
            `${model.dataset_id}::${model.dataset_id}::${model.id}::${view.id}`,
            view.id,
          );
        }
      }
                ...
                for (const nColumn of childModel.columns) {
                  if (
                    ...
                    if (childColumn.id.includes('::')) {
                      ...
                      idMap.set(
                        `${childColumn.project_id}::${childColumn.dataset_id}::${childColumn.fk_model_id}::${childColumn.id}`,
                        nColumn.id,
                      );
                    }
