...
import { setTokenCookie } from 'src/services/users/helpers';
...
export class AuthController {
  ...
  @Post([
    '/auth/user/verification-token',
    '/api/v1/db/auth/user/verification-token',
    '/api/v1/auth/user/verification-token',
  ])
  @UseGuards(PublicApiLimiterGuard)
  @HttpCode(200)
  async emailVerificationToken(@Req() req: Request, @Res() res: Response): Promise<any> {
    const {userId, email} = req.body;
    await this.usersService.emailVerificationToken({
        userId,
        email,
        req
      }
  ...
  async signin(@Req() req: Request, @Res() res: Response) {
    ...
    const {user, token} = await this.usersService.login(req.body, req)
  ...
  async signOut(@Req() req: Request, @Res() res: Response): Promise<any> {
    if (!(req as any).user.isAuthorized) {
