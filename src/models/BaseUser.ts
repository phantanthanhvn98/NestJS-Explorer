import { ProjectRoles } from 'plus0-sdk';
import type { BaseType } from 'plus0-sdk';
...
export default class BaseUser {
  ...
  public static async bulkInsert(
    ...
    const insertObj = baseUsers.map((baseUser) =>
      extractProps(baseUser, ['fk_user_id', 'project_id', 'roles']),
    );
    ...
    const uniqueFks: string[] = [
      ...new Set(bulkData.map((d) => d.project_id)),
    ] as string[];
    ...
    for (const d of bulkData) {
      await NocoCache.set(
        `${CacheScope.BASE_USER}:${d.project_id}:${d.fk_user_id}`,
        d,
      );
  ...
  public static async insert(
    ...
    const res = await this.get(project_id, fk_user_id, ncMeta);

    await NocoCache.appendToList(
      CacheScope.BASE_USER,
      [project_id],
      `${CacheScope.BASE_USER}:${project_id}:${fk_user_id}`,
    );
  ...
  public static async getUsersList(
    ...
    const cachedList = await NocoCache.getList(CacheScope.BASE_USER, [project_id]);
    ...
    if (!isNoneList && !baseUsers.length) {
      ...
      baseUsers = baseUsers.map((baseUser) => {
        baseUser.dataset_id = project_id;
        return this.castType(baseUser);
      });

      await NocoCache.setList(CacheScope.BASE_USER, [project_id], baseUsers, [
        'dataset_id',
        'id',
      ]);
    }
  ...
  static async updateOrInsert(
    ...
    if (existingProjectUser) {
      ...
      return await this.insert({ project_id: baseId, fk_user_id: userId });
    }
  }
}
