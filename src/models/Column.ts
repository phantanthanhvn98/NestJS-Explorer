...
import type { ColumnReqType, ColumnType } from 'plus0-sdk';
...
export default class Column<T = any> implements ColumnType {
  ...
  public static async insert<T>(
    ...
    if (!(column.project_id && column.dataset_id)) {
    ...
    const row = await ncMeta.metaInsert2(
      null, //column.project_id || column.dataset_id,
  ...
  private static async insertColOption<T>(
    column: Partial<T> & { dataset_id?: string; [p: string]: any },
    colId,
    ncMeta = Noco.ncMeta,
  ) {
  ...
  private static async bulkInsertColOption<T>(
    columns: (Partial<T> & { dataset_id?: string; [p: string]: any })[],
    ncMeta = Noco.ncMeta,
  ) {
