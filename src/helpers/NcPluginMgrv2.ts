import { PluginCategory } from 'plus0-sdk';
...
class NcPluginMgrv2 {
  ...
  private static async initPluginsFromEnv() {
    ...
    if (
      process.env.PLUS0_SMTP_FROM &&
      process.env.PLUS0_SMTP_HOST &&
      process.env.PLUS0_SMTP_PORT
    ) {
      const smtpPlugin = await Plugin.getPluginByTitle(SMTPPluginConfig.title);
      await Plugin.update(smtpPlugin.id, {
        active: true,
        input: JSON.stringify({
          from: process.env.PLUS0_SMTP_FROM,
          host: process.env.PLUS0_SMTP_HOST,
          port: process.env.PLUS0_SMTP_PORT,
          username: process.env.PLUS0_SMTP_USERNAME,
          password: process.env.PLUS0_SMTP_PASSWORD,
          secure: process.env.PLUS0_SMTP_SECURE,
          ignoreTLS: process.env.PLUS0_SMTP_IGNORE_TLS,
        }),
      });
    }
  }
