...
export default class NcConnectionMgrv2 {
  ...
  public static async deleteAwait(source: Source) {
    ...
    if (this.connectionRefs?.[source.project_id]?.[source.id]) {
      try {
        const conn = this.connectionRefs?.[source.project_id]?.[source.id];
        await conn.destroy();
        delete this.connectionRefs?.[source.project_id][source.id];
      } catch (e) {
        this.logger.error({
          error: e,
  ...
  public static async get(source: Source): Promise<XKnex> {
    ...
    if (this.connectionRefs?.[source.project_id]?.[source.id]) {
      return this.connectionRefs?.[source.project_id]?.[source.id];
    }
    this.connectionRefs[source.project_id] =
      this.connectionRefs?.[source.project_id] || {};

    const connectionConfig = await source.getConnectionConfig();

    this.connectionRefs[source.project_id][source.id] = XKnex({
      ...defaultConnectionOptions,
      ...connectionConfig,
      connection: {
        ...defaultConnectionConfig,
