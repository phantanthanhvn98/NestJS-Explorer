...
import type { DbConfig, Plus0Config } from '~/interface/config';
...
export default abstract class BaseApiBuilder<T extends Noco> {
  ...
  public async xcUpgrade(): Promise<any> {
    ...
    if (config) {
      const configObj: Plus0Config = JSON.parse(config.value);
      ...
      const configObj: Plus0Config = JSON.parse(JSON.stringify(this.config));
      ...
      await this.xcMeta.metaInsert(this.baseId, this.dbAlias, MetaTable.STORE, {
        key: 'NC_CONFIG',
        value: JSON.stringify(configObj),
      });
      if (isOld) {
