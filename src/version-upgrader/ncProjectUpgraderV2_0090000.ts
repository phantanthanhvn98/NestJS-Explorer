...
import type { UserType, ViewType } from 'plus0-sdk';
...
async function migrateProjectUsers(
  ...
  const baseUsers = await ncMeta.metaList(null, null, MetaTable.PROJECT_USERS);

  for (const baseUser of baseUsers) {
    // skip if base is missing
    if (!(baseUser.project_id in projectsObj)) continue;
    ...
    await BaseUser.insert(
      {
        project_id: baseUser.project_id,
...
async function migrateProjectModels(
  ...
  for (const modelData of metas) {
    ...
    if (modelData.type === 'table' || modelData.type === 'view') {
      ...
      const base = await Base.getWithInfo(modelData.project_id, ncMeta);
...
async function migrateProjectModelViews(
  ...
  for (const viewData of views) {
    const base = await Base.getWithInfo(viewData.project_id, ncMeta);
    // @ts-ignore
    const sourceId = base.sources[0].id;
      ...
      fk_view_id =
        ctx.objViewRef[sharedView.project_id]?.[
          (
            ...
            ctx.objModelAliasRef?.[sharedView.project_id]?.[sharedView.model_name]
          )?.title
...
async function migrateSharedBase(ncMeta: any) {
  ...
  for (const sharedBase of sharedBases) {
    await Base.update(
      ...
      {
        uuid: sharedBase.shared_project_id,
    ...
    const hook = await Hook.insert(
      {
        ...
        dataset_id: hookMeta.project_id,
